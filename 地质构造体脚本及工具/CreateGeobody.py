...
try:
    ...
    outputNewxy ="Newxy"
    TinDomain = "Tindomain"
    Z_Field1 = r"cdbg" #层底标高
    Z_Field = r"cdnbg" #层顶标高
    #arcpy.env.outputCoordinateSystem = inputSR2

    arcpy.MakeXYEventLayer_management(table=inputTable,in_x_field=X_Field,in_y_field=Y_Field,
                                                  out_layer=outputNewxy,spatial_reference=inputSR2,in_z_field=Z_Field)
    arcpy.AddMessage("MakeXYEventLayer_management done...")

    newXYPara = outputNewxy+r"  cdnbg Mass_Points <None>".replace("cdnbg",Z_Field) #层顶标高
    ...
    newXYPara = outputNewxy+r"  cdbg Mass_Points <None>".replace("cdbg",Z_Field1) #层底标高
    ...
    outputFeatures = outputWorkspace+"\\"+"TinExtrude"
