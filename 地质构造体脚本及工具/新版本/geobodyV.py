...
for num, file in enumerate(arcpy.ListFiles()):
    arcpy.env.workspace = r'E:\ArcPyscript\geobodyPY\sourcetable'
    filepath = os.path.join(arcpy.env.workspace, file)
    print(filepath)
    workbook = openpyxl.load_workbook(filepath, read_only=True, data_only=True)
    outputResult = r'E:\ArcPyscript\geobodyPY\result.gdb\MergeGeobody' + str(num)
    ...
    for indexRow in range(1, len(line)):
        ...
        line[indexRow][indexName] = str(line[indexRow][indexName]).replace(r"\n", r"").replace('\n', '')
